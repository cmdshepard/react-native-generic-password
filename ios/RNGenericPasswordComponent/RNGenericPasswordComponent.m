...
#import "RNGenericPasswordComponent.h"
...
+ (GenericPasswordExtension *)sharedExtension {
    ...
    static GenericPasswordExtension *__sharedExtension;
    
    dispatch_once(&onceToken, ^{
        __sharedExtension = [GenericPasswordExtension new];
    });
...
- (void)findLoginForURLString:(NSString *)URLString forViewController:(UIViewController *)viewController sender:(id)sender completion:(void (^)(NSDictionary *loginDictionary, NSError *error))completion {
    ...
    if (![self isSystemAppExtensionAPIAvailable]) {
        ...
        if (completion) {
            completion(nil, [GenericPasswordExtension systemAppExtensionAPINotAvailableError]);
        }
    ...
    activityViewController.completionWithItemsHandler = ^(NSString *activityType, BOOL completed, NSArray *returnedItems, NSError *activityError) {
        if (returnedItems.count == 0) {
            ...
            if (activityError) {
                ...
                error = [GenericPasswordExtension failedToContactExtensionErrorWithActivityError:activityError];
            }
            else {
                error = [GenericPasswordExtension extensionCancelledByUserError];
            }
...
+ (NSError *)systemAppExtensionAPINotAvailableError {
    NSDictionary *userInfo = @{ NSLocalizedDescriptionKey : NSLocalizedString(@"App Extension API is not available is this version of iOS", @"GenericPassword Extension Error Message") };
...
+ (NSError *)extensionCancelledByUserError {
    NSDictionary *userInfo = @{ NSLocalizedDescriptionKey : NSLocalizedString(@"GenericPassword Extension was cancelled by the user", @"GenericPassword Extension Error Message") };
...
+ (NSError *)failedToContactExtensionErrorWithActivityError:(NSError *)activityError {
    ...
    userInfo[NSLocalizedDescriptionKey] = NSLocalizedString(@"Failed to contact the GenericPassword Extension", @"GenericPassword Extension Error Message");
    if (activityError) {
...
+ (NSError *)failedToCollectFieldsErrorWithUnderlyingError:(NSError *)underlyingError {
    NSMutableDictionary *userInfo = [NSMutableDictionary new];
    userInfo[NSLocalizedDescriptionKey] = NSLocalizedString(@"Failed to execute script that collects web page information", @"GenericPassword Extension Error Message");
    if (underlyingError) {
...
+ (NSError *)failedToLoadItemProviderDataErrorWithUnderlyingError:(NSError *)underlyingError {
    NSMutableDictionary *userInfo = [NSMutableDictionary new];
    userInfo[NSLocalizedDescriptionKey] = NSLocalizedString(@"Failed to parse information returned by GenericPassword Extension", @"GenericPassword Extension Error Message");
    if (underlyingError) {
        userInfo[NSUnderlyingErrorKey] = underlyingError;
    }
...
+ (NSError *)failedToObtainURLStringFromWebViewError {
    NSDictionary *userInfo = @{ NSLocalizedDescriptionKey : NSLocalizedString(@"Failed to obtain URL String from web view. The web view must be loaded completely when calling the GenericPassword Extension", @"GenericPassword Extension Error Message") };
    ...
    [itemProvider loadItemForTypeIdentifier:(NSString *)kUTTypePropertyList options:nil completionHandler:^(NSDictionary *loginDictionary, NSError *itemProviderError)
     {
         ...
         if (!loginDictionary) {
             ...
             error = [GenericPasswordExtension failedToLoadItemProviderDataErrorWithUnderlyingError:itemProviderError];
         }
...
- (void)fillLoginIntoWKWebView:(WKWebView *)webView forViewController:(UIViewController *)viewController sender:(id)sender completion:(void (^)(BOOL success, NSError *error))completion {
    ...
    [webView evaluateJavaScript:OPWebViewCollectFieldsScript completionHandler:^(NSString *result, NSError *error) {
        if (!result) {
            NSLog(@"GenericPassword Extension failed to collect web page fields: %@", error);
            if (completion) {
                completion(NO,[GenericPasswordExtension failedToCollectFieldsErrorWithUnderlyingError:error]);
            }
        ...
        __strong __typeof__(self) strongMe = miniMe;
        [strongMe findLoginInGenericPasswordWithURLString:webView.URL.absoluteString collectedPageDetails:result forWebViewController:viewController sender:sender withWebView:webView completion:^(BOOL success, NSError *error) {
            if (completion) {
                completion(success, error);
            }
        }];
...
- (void)fillLoginIntoUIWebView:(UIWebView *)webView webViewController:(UIViewController *)viewController sender:(id)sender completion:(void (^)(BOOL success, NSError *error))completion {
    ...
    [self findLoginInGenericPasswordWithURLString:webView.request.URL.absoluteString collectedPageDetails:collectedPageDetails forWebViewController:viewController sender:sender withWebView:webView completion:^(BOOL success, NSError *error) {
        if (completion) {
            completion(success, error);
        }
    }];
...
- (void)findLoginInGenericPasswordWithURLString:(NSString *)URLString collectedPageDetails:(NSString *)collectedPageDetails forWebViewController:(UIViewController *)forViewController sender:(id)sender withWebView:(id)webView completion:(void (^)(BOOL success, NSError *error))completion {
    if ([URLString length] == 0) {
        NSError *URLStringError = [GenericPasswordExtension failedToObtainURLStringFromWebViewError];
        NSLog(@"Failed to findLoginInGenericPasswordWithURLString: %@", URLStringError);
    ...
    activityViewController.completionWithItemsHandler = ^(NSString *activityType, BOOL completed, NSArray *returnedItems, NSError *activityError) {
        if (returnedItems.count == 0) {
            NSError *error = nil;
            if (activityError) {
                NSLog(@"Failed to findLoginInGenericPasswordWithURLString: %@", activityError);
                error = [GenericPasswordExtension failedToContactExtensionErrorWithActivityError:activityError];
            }
            else {
                error = [GenericPasswordExtension extensionCancelledByUserError];
...
- (void)executeFillScript:(NSString *)fillScript inWebView:(id)webView completion:(void (^)(BOOL success, NSError *error))completion {
    if (!fillScript) {
        ...
        if (completion) {
            completion(NO, [GenericPasswordExtension failedToFillFieldsErrorWithLocalizedErrorMessage:NSLocalizedString(@"Failed to fill web page because script is missing", @"GenericPassword Extension Error Message") underlyingError:nil]);
        }
    ...
    if ([webView isKindOfClass:[UIWebView class]]) {
        ...
        if (!success) {
            ...
            error = [GenericPasswordExtension failedToFillFieldsErrorWithLocalizedErrorMessage:NSLocalizedString(@"Failed to fill web page because script could not be evaluated", @"GenericPassword Extension Error Message") underlyingError:nil];
        }
    ...
    if ([webView isKindOfClass:[WKWebView class]]) {
        [((WKWebView *)webView) evaluateJavaScript:scriptSource completionHandler:^(NSString *result, NSError *evaluationError) {
            ...
            if (!success) {
                ...
                error = [GenericPasswordExtension failedToFillFieldsErrorWithLocalizedErrorMessage:NSLocalizedString(@"Failed to fill web page because script could not be evaluated", @"GenericPassword Extension Error Message") underlyingError:error];
            }
